...
export class StocksService {
  ...
  async create(createStockDto: CreateStockDto) {
    ...
    if (existing) {
      throw new ConflictException('Bu kitap, dil ve depo kombinasyonu için zaten bir stok kaydı var.');
    }
  ...
  async findAll(bookName?: string, languageId?: string, warehouseId?: string, lowStock?: string) {
    try {
      const queryBuilder = this.stockRepository.createQueryBuilder('stock')
        ...
        .addSelect(['bookTranslations.id', 'bookTranslations.title', 'bookTranslations.languageId']) // Sadece gerekli alanları seç
