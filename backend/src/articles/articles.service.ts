...
export class ArticlesService {
  ...
  async update(id: number, updateArticleDto: UpdateArticleDto): Promise<Article> {
    ...
    if (translations && Array.isArray(translations)) {
      await this.articleTranslationRepository.delete({ articleId: id });
      
      // Mevcut slug'ları al (güncellenen makale hariç)
      const existingSlugs = await this.getExistingSlugs();
      ...
      await this.articleTranslationRepository.save(newTranslations);
      updatedArticle.translations = newTranslations;
    }
  ...
  async reorderArticles(bookId: number, articleOrders: { id: number; orderIndex: number }[]): Promise<void> {
