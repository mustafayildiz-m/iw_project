...
export class CreateStoryViewsAndLikesTables1759501474092 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`story_views\` (\`id\` int NOT NULL AUTO_INCREMENT, \`story_id\` int NOT NULL, \`user_id\` int NOT NULL, \`viewed_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_3b8ffe2eee1959849d3dd21bc3\` (\`story_id\`, \`user_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`story_likes\` (\`id\` int NOT NULL AUTO_INCREMENT, \`story_id\` int NOT NULL, \`user_id\` int NOT NULL, \`liked_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_4c05956e2f1fe1a23339a255cc\` (\`story_id\`, \`user_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`story_views\` ADD CONSTRAINT \`FK_e30910f5c5e7cefc96f92d17331\` FOREIGN KEY (\`story_id\`) REFERENCES \`scholar_stories\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`story_views\` ADD CONSTRAINT \`FK_957569250010b2ec66127999d15\` FOREIGN KEY (\`user_id\`) REFERENCES \`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`story_likes\` ADD CONSTRAINT \`FK_a68f51aa0d901bbc019c65572b3\` FOREIGN KEY (\`story_id\`) REFERENCES \`scholar_stories\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`IDX_3b8ffe2eee1959849d3dd21bc3\` ON \`story_views\``);
