...
export class CreateScholarStoriesTable1759498383957 implements MigrationInterface {
    name = 'CreateScholarStoriesTable1759498383957'

    // Helper function to safely drop column
    private async safeDropColumn(queryRunner: QueryRunner, tableName: string, columnName: string): Promise<void> {
        try {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        // Drop foreign key if it exists (handle case where it might not exist)
        try {
        ...
        // Safely drop columns from books table
        ...
        // Safely drop columns from book_languages table
        await this.safeDropColumn(queryRunner, 'book_languages', 'pdfUrl');
        
        // Safely drop columns from languages table
        ...
        // Add columns back (using IF NOT EXISTS logic)
        try {
        ...
        // Handle scholars.biography column
        try {
