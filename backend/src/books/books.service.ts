...
export class BooksService {
  ...
  async create(createBookDto: CreateBookDto): Promise<Book> {
    ...
    if (category && Array.isArray(category)) {
      const bookCategories = category.map((catName: string) =>
        this.bookCategoryRepository.create({ bookId: savedBook.id, categoryName: catName })
      );
  ...
  async findAll(languageId?: string, search?: string, category?: string, page: number = 1, limit: number = 12): Promise<any> {
  ...
  async update(id: number, updateBookDto: CreateBookDto): Promise<Book | null> {
    ...
    if (category && Array.isArray(category)) {
      ...
      const newCategories = category.map((catName: string) =>
          this.bookCategoryRepository.create({ bookId: id, categoryName: catName }),
      );
      await this.bookCategoryRepository.save(newCategories);
    }

    // 🧹 Eski çevirileri sil ve yenilerini kaydet
    ...
    if (translations && Array.isArray(translations)) {
      ...
      await this.bookTranslationRepository.save(newTranslations);
      updatedBook.translations = newTranslations;
    }
