import {
  ...
  UploadedFiles,
  UseGuards,
  ParseIntPipe, NotFoundException, Query,
} from '@nestjs/common';
...
export class BooksController {
  ...
  @Get('public/:id')
  findOnePublic(@Param('id', ParseIntPipe) id: number, @Query('lang') lang?: string) {
  ...
  async create(
    ...
    if (createBookDto.translations && Array.isArray(createBookDto.translations)) {
      createBookDto.translations = await Promise.all(createBookDto.translations.map(async (trans: any, idx: number) => {
  ...
  async update(
    ...
    // 📄 Çevirilerin PDF dosyalarını eşle
    if (updateBookDto.translations && Array.isArray(updateBookDto.translations)) {
      updateBookDto.translations = await Promise.all(updateBookDto.translations.map(async (trans: any, idx: number) => {
        const pdfFile = files.find(f => f.fieldname === `translations[${idx}][pdfFile]`);
        if (pdfFile) {
          return {
            ...trans,
