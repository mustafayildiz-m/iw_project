import { Controller, Get, Post, Body, Patch, Param, Delete, UseGuards, UploadedFiles, UseInterceptors, Request, BadRequestException, Query } from '@nestjs/common';
...
export class ScholarsController {
  ...
  async create(
    ...
    if (createScholarDto.ownBooks && Array.isArray(createScholarDto.ownBooks)) {
      createScholarDto.ownBooks = await Promise.all(createScholarDto.ownBooks.map(async (book: any, idx: number) => {
  ...
  @Get(':id')
  findOne(@Param('id') id: string, @Query('userId') queryUserId: string, @Request() req: any) {
  ...
  async update(
    ...
    if (updateScholarDto.ownBooks && Array.isArray(updateScholarDto.ownBooks)) {
      updateScholarDto.ownBooks = await Promise.all(updateScholarDto.ownBooks.map(async (book: any, idx: number) => {
        const coverFile = files.find(f => f.fieldname === `ownBooks[${idx}][cover]`);
        const pdfFile = files.find(f => f.fieldname === `ownBooks[${idx}][pdf]`);
