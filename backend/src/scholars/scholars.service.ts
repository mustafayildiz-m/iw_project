...
export class ScholarsService {
  ...
  async create(createScholarDto: CreateScholarDto, userId?: number) {
    if (typeof createScholarDto.birthDate === 'string' && (createScholarDto.birthDate === '' || createScholarDto.birthDate === 'null')) createScholarDto.birthDate = undefined;
    if (typeof createScholarDto.deathDate === 'string' && (createScholarDto.deathDate === '' || createScholarDto.deathDate === 'null')) createScholarDto.deathDate = undefined;
  ...
  async findAll(userId?: number, page: number = 1, limit: number = 10, search?: string) {
    ...
    if (search && search.trim()) {
      ...
      queryBuilder.where(
        '(scholar.fullName LIKE :search OR scholar.lineage LIKE :search OR scholar.biography LIKE :search)',
        { search: searchTerm }
      );
    }
  ...
  async getTotalCount(search?: string): Promise<number> {
    // Arama varsa filtrelenerek sayalım
    if (search && search.trim()) {
      const searchTerm = `%${search.trim()}%`;
  ...
  async update(id: number, updateScholarDto: UpdateScholarDto, userId?: number) {
    const nullify = (val: any) => (val === '' || val === 'null' ? undefined : val);
