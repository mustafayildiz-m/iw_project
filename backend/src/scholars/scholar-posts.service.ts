...
import { CreateScholarPostDto, CreateTranslationDto } from './dto/create-scholar-post.dto';
...
export class ScholarPostsService {
  ...
  async create(createScholarPostDto: CreateScholarPostDto): Promise<ScholarPost> {
  ...
  async update(id: string, updateDto: UpdateTranslationDto): Promise<ScholarPost> {
    const post = await this.scholarPostRepository.findOne({ 
      where: { id },
      relations: ['translations']
    });
