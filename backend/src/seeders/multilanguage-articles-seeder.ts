...
@Injectable()
export class MultiLanguageArticlesSeeder {
  constructor(
      ...
      try {
        ...
        for (const [langCode, content] of Object.entries(articleInfo.translations)) {
          ...
          if (language && content && typeof content === 'object' && 'title' in content) {
            const translationContent = content as { title: string; content: string; summary: string };
            const slug = createUniqueSlug(translationContent.title, existingSlugs);
            existingSlugs.push(slug); // Yeni slug'ı listeye ekle
  ...
  private getArticleData() {
    ...
    // Daha fazla makale verisi ekleyelim (toplam 70 makale için)
  ...
  private generateAdditionalArticles() {
    ...
    for (let i = 0; i < 65; i++) { // 5 tanesi zaten var, 65 tane daha ekleyelim
  ...
  private generateTurkishContent(title: string): string {
  ...
  private generateEnglishContent(title: string): string {
  ...
  private generateArabicContent(title: string): string {
