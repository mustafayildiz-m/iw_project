...
export class ChatService {
  ...
  async getConversationMessages(conversationId: string, userId: number, limit = 50, offset = 0): Promise<MessageResponse[]> {
  ...
  async getOnlineUsers(): Promise<Array<{id: number, username: string, photoUrl?: string}>> {
    // Online kullanıcıların ID, username ve photoUrl'lerini döndür
    const onlineUsers: Array<{id: number, username: string, photoUrl?: string}> = [];
  ...
  async searchMessages(userId: number, query: string): Promise<MessageResponse[]> {
    const messages = await this.messageRepository
      .createQueryBuilder('message')
      .leftJoinAndSelect('message.sender', 'sender')
      .leftJoinAndSelect('message.receiver', 'receiver')
  ...
  async deleteConversation(conversationId: string, userId: number): Promise<{ wasHardDeleted: boolean; deletedBy: number; deletedByUsername: string; conversation?: Conversation }> {
