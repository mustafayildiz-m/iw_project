...
export class PodcastService {
  ...
  async create(createPodcastDto: CreatePodcastDto, audioFile?: string, coverFile?: string): Promise<Podcast> {
    try {
      // Tip dönüşümleri yap
  ...
  async update(id: number, updatePodcastDto: UpdatePodcastDto, audioFile?: string, coverFile?: string): Promise<Podcast> {
    ...
    if (updatePodcastDto.description !== undefined) podcast.description = updatePodcastDto.description;
    if (updatePodcastDto.author !== undefined) podcast.author = updatePodcastDto.author;
    if (updatePodcastDto.language) podcast.language = updatePodcastDto.language;
    if (updatePodcastDto.category !== undefined) podcast.category = updatePodcastDto.category;
    if (updatePodcastDto.publishDate) podcast.publishDate = new Date(updatePodcastDto.publishDate);
