...
export class WhoToFollowService {
  ...
  async getWhoToFollow(
    ...
    if (type === 'all' || type === 'users') {
      ...
      const userItems: WhoToFollowItem[] = await Promise.all(users.map(async (user) => {
    ...
    if (type === 'all' || type === 'scholars') {
      ...
      const scholarItems: WhoToFollowItem[] = await Promise.all(scholars.map(async (scholar) => {
        let isFollowing = false;
        if (userId) {
          try {
  ...
  async searchUsersCount(searchQuery: string, userId?: number): Promise<number> {
    const query = this.userRepository
      .createQueryBuilder('user')
      .where('user.isActive = :isActive', { isActive: true })
      .andWhere(
