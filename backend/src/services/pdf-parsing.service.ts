...
import { PDFSpecificScholarParser, ScholarData, ParserConfig } from './enhanced-pdf-parser';
...
export class PDFParsingService {
   ...
   */
  async processPDF(filePath: string, userId?: string): Promise<{ jobId: string }> {
    try {
   ...
   */
  async parseScholarsPDF(filePath: string, config?: Partial<ParserConfig>): Promise<{
    ...
    try {
      ...
      const parser = new PDFSpecificScholarParser(filePath, outputPath, parserConfig);
  ...
  async testParser(sampleText?: string): Promise<any> {
    ...
    try {
      ...
      const parser = new PDFSpecificScholarParser(testFilePath, outputPath, this.getParserConfig());
