...
export class UserFollowService {
  ...
  async follow(follower_id: number, following_id: number) {
    if (follower_id === following_id) throw new NotFoundException('Kendini takip edemezsin.');
  ...
  async getFollowingUsers(userId: number, limit: number = 20, offset: number = 0) {
  ...
  async getRecentPostsFromFollowing(userId: number, limit: number = 5, language: string = 'tr') {
    try {
  ...
  private async getScholarPosts(userId: number, limit: number, language: string = 'tr') {
    try {
