...
export class LanguageService {
  ...
  async getBookCounts(): Promise<{ languageId: number; languageName: string; languageCode: string; bookCount: number }[]> {
  ...
  async getArticleCounts(): Promise<{ languageId: number; languageName: string; languageCode: string; articleCount: number }[]> {
    const result = await this.languageRepository
      .createQueryBuilder('language')
  ...
  async update(
    ...
    try {
      ...
      if (updateLanguageDto.name) {
        ...
        if (duplicateName) {
          throw new ConflictException('Bu dil adı başka bir dilde kullanılıyor.');
        }
      }
      ...
      if (updateLanguageDto.code) {
        ...
        if (duplicateCode) {
          throw new ConflictException('Bu dil kodu başka bir dilde kullanılıyor.');
        }
      }
