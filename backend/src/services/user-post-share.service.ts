...
export class UserPostShareService {
  ...
  async sharePost(userId: number, postId: string, postType: 'user' | 'scholar' = 'user') {
  ...
  async unsharePost(userId: number, postId: string, postType: 'user' | 'scholar' = 'user') {
  ...
  async getPostShareCount(postId: string, postType: 'user' | 'scholar' = 'user') {
  ...
  async isPostSharedByUser(userId: number, postId: string, postType: 'user' | 'scholar' = 'user') {
    const share = await this.userPostShareRepository.findOne({
      where: { user_id: userId, post_id: postId, post_type: postType }
    });
