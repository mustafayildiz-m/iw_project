...
export class IslamicNewsService {
  ...
  // Yeni haber oluştur
  async create(createIslamicNewsDto: CreateIslamicNewsDto): Promise<IslamicNews> {
    const news = this.islamicNewsRepository.create(createIslamicNewsDto);
    return this.islamicNewsRepository.save(news);
  }

  // Tüm haberleri getir
  async findAll(limit: number = 20, offset: number = 0): Promise<[IslamicNews[], number]> {
  ...
  // Haber güncelle
  async update(id: number, updateIslamicNewsDto: UpdateIslamicNewsDto): Promise<IslamicNews | null> {
  ...
  private async saveArchiveNewsToDatabase(newsArray: any[]): Promise<IslamicNews[]> {
    const savedNews: IslamicNews[] = [];
  ...
  async getNewsFromDatabase(
    ...
    const cachedResult = await this.cacheService.get<[IslamicNews[], number]>(cacheKey);
    if (cachedResult) {
