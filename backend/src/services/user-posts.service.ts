...
export class UserPostsService {
  ...
  async getTimeline(userId: number, language: string = 'tr') {
    // Cache key oluştur - shared posts ve dil için v3
    const cacheKey = `user-posts:timeline:${userId}:${language}:v3`;
    ...
    allUserPosts.sort((a, b) => {
      const aDate = (a as any).shared_at ? new Date((a as any).shared_at) : new Date(a.created_at);
      const bDate = (b as any).shared_at ? new Date((b as any).shared_at) : new Date(b.created_at);
    ...
    for (const share of sharedPosts) {
      if (share.post_type === 'user') {
        ...
        const originalPost = await this.scholarPostRepository.findOne({
          where: { id: share.post_id as any },
          relations: ['scholar', 'translations']
        });
        if (originalPost) {
    ...
    scholarPosts.sort((a, b) => {
      const aDate = (a as any).shared_at ? new Date((a as any).shared_at) : new Date(a.createdAt);
      const bDate = (b as any).shared_at ? new Date((b as any).shared_at) : new Date(b.createdAt);
      return bDate.getTime() - aDate.getTime();
    });
    ...
    const normalizedUserPosts = await Promise.all(allUserPosts.map(async p => {
      const user = userMap.get(p.user_id);
      
      // Yorum sayısını al
      const commentCount = await this.userPostCommentRepository.count({
        where: { post_id: p.id }
      });
    ...
    // Cache'e kaydet
    await this.cacheService.set(cacheKey, allPosts, 300); // 5 dakika cache süresi
  ...
  private async clearTimelineCacheForUser(userId: number) {
    ...
    for (const lang of languages) {
      const cacheKey = `user-posts:timeline:${userId}:${lang}:v3`;
    ...
    for (const follower of followers) {
      for (const lang of languages) {
        const cacheKey = `user-posts:timeline:${follower.follower_id}:${lang}:v3`;
        await this.cacheService.del(cacheKey);
      }
    }
  }
