...
export default function ArticleGrid({ 
  ...
  const getArticleImage = (article) => {
    ...
    if (article.coverImage) {
      return article.coverImage.startsWith('http') ? article.coverImage : `${process.env.NEXT_PUBLIC_API_URL}${article.coverImage}`;
    }
    
    // Makale coverImage'ı yoksa, kitabın coverImage veya coverUrl'ini kullan
    if (article.book) {
      if (article.book.coverImage) {
        return article.book.coverImage.startsWith('http') ? article.book.coverImage : `${process.env.NEXT_PUBLIC_API_URL}${article.book.coverImage}`;
      }
      if (article.book.coverUrl) {
        return article.book.coverUrl.startsWith('http') ? article.book.coverUrl : `${process.env.NEXT_PUBLIC_API_URL}${article.book.coverUrl}`;
      }
    }
              ...
              <Col key={article.id || idx} xs={12} md={6} lg={4} className="mb-4">
                <Card className="h-100 shadow-sm border-0 article-card">
                  ...
                  <Card.Body className="d-flex flex-column">
                    <Link href={getArticleDetailUrl(article.id)} className="text-decoration-none">
      ...
      {viewMode === 'list' && (
        <div className="mb-4">
          {articles.map((article, idx) => {
            ...
            return (
              <Card key={article.id || idx} className="mb-3 shadow-sm border-0 article-list-card" style={{
