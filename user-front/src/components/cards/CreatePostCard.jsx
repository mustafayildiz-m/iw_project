      ...
      <div className="mt-4">
        <div className="d-flex align-items-center mb-3">
          ...
          <h6 className="mb-0 fw-bold text-success">
            {fileUrls.length > 1 ? `${fileUrls.length} dosya seçildi` : '1 dosya seçildi'}
          </h6>
        </div>
        <div className="row g-3">
          {fileUrls.map(({ file, url }, index) => (
            <div key={`${file.name}-${index}`} className="col-12">
              <div className="file-preview-card p-3 bg-white rounded-3 border shadow-sm position-relative">
                <button
                  type="button"
                ...
                <div className="d-flex align-items-center">
                  <div className="icon-shape icon-sm rounded-circle bg-light me-3">
                    {file.type.startsWith('image/') ? (
                      <BsImageFill className="text-success" size={16} />
                    ) : (
                      <BsCameraReelsFill className="text-info" size={16} />
                    )}
                  </div>
                  <div className="flex-grow-1">
                    <h6 className="mb-1 text-truncate">{file.name}</h6>
                    <div className="d-flex align-items-center gap-3">
                      <small className="text-muted">
                        {(file.size / 1024 / 1024).toFixed(2)} MB
                      </small>
                      {file.type.startsWith('video/') && (file.size / (1024 * 1024)) > 50 && (
                        <small className="badge bg-warning-soft text-warning">
                          Büyük dosya
                        </small>
                      )}
                    </div>
